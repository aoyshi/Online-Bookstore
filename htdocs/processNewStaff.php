...
$name_error = $email_error = $username_error = $password_error = $phone_error = $address_error = $DOB_error = $ssn_error = $salary_error = "";
$name = $email = $username = $password = $phone = $address = $DOB = $sex = $ssn = $salary = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["DOB"])) {
	...
	if(!preg_match('/^\d+$/', $ssn)){
	   $ssn_error = "Invalid SSN";
	}
    if(strlen($ssn)!=9){
	   $ssn_error = "SSN must be 9 digits";
	}
  }
	...
	if(!preg_match('/^\d+$/', $salary)){
	   $salary_error = "Invalid salary entry";
	}
  }

	//check if all error-free before inserting
	if ($name_error == '' and $email_error == '' and $username_error == '' and $password_error == '' and $phone_error == '' 
	    and $address_error == '' and $DOB_error == '' and $ssn_error == '' and $salary_error == '')
	{			
		$hostname = "localhost";
		$dbuser="root";
		$dbpass="root";
		...
		`Password`, `DOB`, `Sex`, `Role`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'STAFF');";			    
		...
		$query = "INSERT INTO `staff` (`StaffID`, `SSN`, `Salary`) VALUES (?, ?, ?);";    
		$stmt = mysqli_prepare($conn, $query);
		mysqli_stmt_bind_param($stmt,"sii",$username,$ssn,$salary);	
		mysqli_stmt_execute($stmt);
		
		$affected_rows = mysqli_stmt_affected_rows($stmt);
		if($affected_rows == 1)
		{
			echo 'Insertion Successful in STAFF!<br>';
			mysqli_stmt_close($stmt);
			mysqli_close($conn);
		}
		else
		{
			echo 'Error Occured during Insert in STAFF: ' . mysqli_error();
			mysqli_stmt_close($stmt);
			mysqli_close($conn);
		}
