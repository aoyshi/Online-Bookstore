...
<div class="myMain">
  <?php
	...
	$query = "select *
	          from item,genre,book 
			  where Genre_ID=ID and ItemID=BookID and Availability='1';";
	$response = @mysqli_query($conn,$query);
	if($response) 
	{
		echo '<h3>ALL AVAILABLE BOOKS</h3>';
	...
	$query = "select *
	          from item,genre,book 
			  where Genre_ID=ID and ItemID=BookID and Availability='0';";
	$response = @mysqli_query($conn,$query);
	if($response) 
	{
		echo '<h3>UNAVAILABLE BOOKS</h3>';
		echo '<table cellspacing="5" cellpadding="5">
				 <tr><td><b>ItemID</b></td>
				     <td><b>Title</b></td>
					 <td><b>Genre</b></td>
