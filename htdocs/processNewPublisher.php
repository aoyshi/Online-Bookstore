  ...
  if (empty($_POST["name"])) {
   ...
   if (empty($_POST["location"])) {
	...
	if (strlen($location)>256) {
	  $location_error = "Location entry cannot exceed 256 characters";
	}	  
  }

  if (empty($_POST["DOB"])) {
    $DOB_error = "Founding Date is required";
  } else {
	...
	if ($name_error == '' and $id_error == '' and $location_error == '' and $DOB_error == '')
	{
	...
	$conn = @mysqli_connect($hostname,$dbuser,$dbpass,$db)
		...
		$query = "INSERT INTO `creator` (`Name`, `ID`, `Classification`) VALUES ('".$name."','".$id."','Publisher');";	
		...
		$query2 = "INSERT INTO `publisher` (`PubID`, `Location`, `Date_Founded`) 
		          VALUES ('".$id."', '".$location."', '".$DOB."');";  
		$result2 = mysqli_query($conn, $query2);
		if($result2)
		{
			echo 'Insertion in PUBLISHER Successful!<br>';
			mysqli_close($conn);
		}
		else
		{
			echo 'Error Occured during Insert in PUBLISHER: ' . mysqli_error($conn);
			mysqli_close($conn);
		}
