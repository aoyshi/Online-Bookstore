...
<div class="myMain">
  <?php
	...
	$query = "select ItemID,Item_Name,Category,Name,Price,Description,Language
	          from item,genre 
			  where Genre_ID=ID and Availability='1';";
	$response = @mysqli_query($conn,$query);
	if($response) 
	{
		echo '<h3>ALL AVAILABLE ITEMS</h3>';
	...
	$query = "select ItemID,Item_Name,Category,Name,Price,Description,Language
	          from item,genre 
			  where Genre_ID=ID and Availability='0';";
	$response = @mysqli_query($conn,$query);
	if($response) 
	{
		echo '<h3>UNAVAILABLE ITEMS</h3>';
		echo '<table cellspacing="5" cellpadding="5">
				 <tr><td><b>ItemID</b></td>
				     <td><b>Title</b></td>
					 <td><b>Category</b></td>
